...
lexer.readWord = function(STREAM,INDEX,LINE,COLUMN)
	...
	while index <= #STREAM do
		character = string.sub(STREAM,index,index);
		if helper.filterString(character,lexer.TABLE.WORD_EXTENDED) == false then
			break;
		end;
	...
	if helper.filterArray({lexeme},lexer.TABLE.KEYWORD) == true then
		return lexer.createToken(string.upper(lexeme),lexeme,LINE,COLUMN),index,column;
	elseif lexeme == "false" then
		return lexer.createToken("BOOLEAN",lexeme,LINE,COLUMN,false),index,column;
	elseif lexeme == "true" then
		return lexer.createToken("BOOLEAN",lexeme,LINE,COLUMN,true),index,column;
	else
		return lexer.createToken("WORD",lexeme,LINE,COLUMN,lexeme),index,column;
	end;
end;
...
lexer.readNumber = function(STREAM,INDEX,LINE,COLUMN)
	...
	while index <= #STREAM do
		character = string.sub(STREAM,index,index);
		if helper.filterString(character,lexer.TABLE.NUMBER_EXTENDED) == false then
			break;
		end;
		lexeme = lexeme .. character;
		index = index + 1;
	...
	if value ~= nil then
		return lexer.createToken("NUMBER",lexeme,LINE,COLUMN,value),index,column;
	else
		return lexer.createToken("INVALID_NUMBER",lexeme,LINE,COLUMN),index,column;
	end;
end;
...
lexer.readString = function(STREAM,INDEX,LINE,COLUMN)
	...
	if character == "\"" then
		return lexer.createToken("STRING",lexeme,LINE,COLUMN,value),index,column;
	else
		return lexer.createToken("INVALID_STRING",lexeme,LINE,COLUMN),index,column;
	end;
end;
